import { useEtapeModele } from "../../../hooks/modeles/useEtapeModele";
...
export default function EtapeForm({
  ...
  const { etapeModele, createEtapeModele, updateEtapeModele, setEtapeModele } = useEtapeModele();
  ...
  const onSubmit = (data, e) => {
    e.preventDefault();
    if (update) {
      updateEtapeModele(etapeData.id, dirtyValues(touchedFields, data))
        .then((response) => {
          setSuccess(true);
      ...
      createEtapeModele({ fiche_modele: ficheId, ...data }).then((response) => {
        setSuccess(true);
        setMessage("Etape créée avec succès");
        setShowAlert(true);
      }).catch((error) => {
        setSuccess(false);
        setMessage("Erreur lors de la création de l'etape.");
        setShowAlert(true);
      });
    }
  };
                ...
                <div className="flex flex-wrap -mx-4">
                  ...
                  <div className={"px-4 pb-2 relative w-full " + widths[12]}>
                    ...
                    <Input
                      ...
                      defaultValue={etapeModele?.description}
                    ...
                    {update && (
                      <label className="inline-flex items-center cursor-pointer">
                        <input
                          {...register("terminee")}
                          type="checkbox"
                          defaultChecked={etapeModele?.terminee}
                          className="mr-3"
                        />
                        Étape terminée ?
                      </label>
